...
export class RecievedOrdersComponent {
  ...
  showFilterDialog() {
    const dialog = Swal.fire({
      ...
      customClass: {
        confirmButton:
          'bg-[#818AA1] text-white font-medium py-2 px-5 rounded-md text-lg mx-2',
        cancelButton:
          'bg-gray-200 text-gray-800 font-medium py-2 px-5 rounded-md text-lg mx-2',
      },
      didOpen: () => {
        ...
        const validateInputs = () => {
          if (confirmButton) {
            confirmButton.disabled = !(
              filterTypeSelect.value && filterDateInput.value
            );
          }
        };
      ...
      preConfirm: () => {
        ...
        const date = (
          document.getElementById('filterDateInput') as HTMLInputElement
        ).value;
