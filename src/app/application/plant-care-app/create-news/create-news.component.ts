...
export class CreateNewsComponent {
      ...
      if (result.isConfirmed) {
        ...
        this.newsService.createNews(formData).subscribe({
          ...
          error: (error: any) => {
            this.isLoading = false;
            let errorMessage =
              'The selected image is too large. The maximum allowed size is 5MB';
  ...
  private buildSnapshot(): string {
    ...
    const item = this.newsItems[0];
    return JSON.stringify({
      titleEnglish: item.titleEnglish,
      descriptionEnglish: item.descriptionEnglish,
      titleSinhala: item.titleSinhala,
      descriptionSinhala: item.descriptionSinhala,
      titleTamil: item.titleTamil,
      descriptionTamil: item.descriptionTamil,
      publishDate: this.formatDateForBackend(this.originalPublishDateEdit),
      expireDate: this.formatDateForBackend(this.originalExpireDateEdit),
      hasNewImage: !!this.selectedFile,
    });
  }

  hasUnsavedChanges(): boolean {
    if (this.itemId === null) return true;
    return this.buildSnapshot() !== this.originalNewsSnapshot;
  }
  ...
  updateNews() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.http
          .put(
            `${environment.API_URL}auth/edit-news/${this.itemId}`,
            formData,
            { headers },
          )
          .subscribe({
            ...
            error: (error: any) => {
              ...
              if (error.error && error.error.error) {
                switch (error.error.error) {
                  ...
                  case 'Duplicate news title':
                    errorMessage =
                      'A news item with this title already exists.';
                    break;
                  default:
                    errorMessage =
  ...
  checkExpireDate() {
    ...
    if (this.originalExpireDate) {
      ...
      if (this.originalPublishDate) {
        ...
        if (
          this.originalExpireDate.getTime() ===
          this.originalPublishDate.getTime()
        ) {
          setTimeout(() => {
            this.originalExpireDate = null;
          });
      ...
      if (this.originalExpireDate < this.todayDate) {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Invalid Data Inputs',
          html: 'Expire date cannot be in the past.',
          confirmButtonText: 'OK',
          customClass: {
            popup: 'bg-tileLight dark:bg-tileBlack text-black dark:text-white',
            title: 'font-semibold text-lg',
    ...
    if (this.originalExpireDateEdit) {
      ...
      if (this.originalPublishDateEdit) {
        if (this.originalExpireDateEdit < this.originalPublishDateEdit) {
          setTimeout(() => {
            this.originalExpireDateEdit = null;
          });
          Swal.fire({
            icon: 'error',
            title: 'Invalid Data Inputs',
        ...
        if (
          this.originalExpireDateEdit.getTime() ===
          this.originalPublishDateEdit.getTime()
        ) {
          setTimeout(() => {
            this.originalExpireDateEdit = null;
          });
      ...
      if (this.originalExpireDateEdit < this.todayDate) {
        setTimeout(() => {
          this.originalExpireDateEdit = null;
        });
        Swal.fire({
  ...
  onTitlePaste(event: ClipboardEvent, fieldName: string): void {
    ...
    // Update the model
    if (this.itemId === null) {
      (this.createNewsObj as any)[fieldName] = newValue;
    } else {
      (this.newsItems[0] as any)[fieldName] = newValue;
    }
