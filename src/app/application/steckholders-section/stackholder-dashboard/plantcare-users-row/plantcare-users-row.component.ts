  ...
  fetchPlantCareUserData() {
    ...
    this.stakeholderSrv.getPlantCareUserData().subscribe(
      (res) => {
        ...
        this.plantCareUsersWithOutQr =
          res.plantCareUserByQrRegistration[1]?.user_count ?? 0;
        this.plantCareUsersWithQr =
          res.plantCareUserByQrRegistration[0]?.user_count ?? 0;
        this.newPlantCareUsers =
          res.newPlantCareUsers[0]?.newPlantCareUserCount ?? 0;
        this.allPlantCareUsers =
          res.allPlantCareUsers[0]?.totalPlantCareUserCount ?? 0;
        this.activePlantCareUsers =
          res.activePlantCareUsers[0]?.activePlantCareUserCount ?? 0;
      },
      (error) => {
