...
export class ReadyToPikupComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['centerObj'] && this.centerObj && this.centerObj.centerId > 0) {
      this.fetchReadyToPickupOrders();
    }
  }

  // FETCH ONLY READY TO PICKUP ORDERS
  fetchReadyToPickupOrders(): void {
    this.isLoading = true;
      ...
      .subscribe({
        next: (res) => {
          if (res && res.data) {
            ...
            // CRITICAL: Filter for ONLY READY TO PICKUP orders on client side
            const readyToPickupData = this.filterOnlyReadyToPickup(dataArray);
            
            // Transform only the filtered data
            this.orders = this.transformApiData(readyToPickupData);
  ...
  // Filter ONLY for Ready to Pickup orders (exclude Picked Up)
  private filterOnlyReadyToPickup(apiData: any[]): any[] {
    return apiData.filter(item => {
      const status = (item.status || item.orderStatus || '').toLowerCase().trim();
      
      // Include only orders with "ready" status (not "picked up")
