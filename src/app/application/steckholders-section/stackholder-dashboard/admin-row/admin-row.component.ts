...
export class AdminRowComponent implements OnChanges {
  ...
  fetchAdminUserData(data: any) {
    this.associateAdmins =
      data.adminUsersByPosition?.Associate?.adminUserCount ?? 0;
    this.executiveAdmins =
      data.adminUsersByPosition?.Executive?.adminUserCount ?? 0;
    this.managerAdmins =
      data.adminUsersByPosition?.Manager?.adminUserCount ?? 0;
    this.officerAdmins =
      data.adminUsersByPosition?.Officer?.adminUserCount ?? 0;
