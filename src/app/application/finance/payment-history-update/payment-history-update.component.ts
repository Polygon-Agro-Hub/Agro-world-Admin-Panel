...
export class PaymentHistoryUpdateComponent implements OnInit {
  ...
  onPaymentReferenceInput(event: Event): void {
    ...
    if (input.value.startsWith(' ')) {
      ...
      setTimeout(() => {
        const newCursorPosition = Math.max(
          0,
          (cursorPosition || 0) - (input.value.length - newValue.length),
        );
    ...
    if (input.value.length > this.MAX_REFERENCE_LENGTH) {
      ...
      const truncatedValue = input.value.substring(
        0,
      ...
      if (input.value.length > this.MAX_REFERENCE_LENGTH + 5) {
        // Show warning after 5 extra characters
