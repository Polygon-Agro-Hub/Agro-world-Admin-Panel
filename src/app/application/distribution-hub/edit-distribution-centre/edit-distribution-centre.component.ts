...
export class EditDistributionCentreComponent implements OnInit {
  ...
  private numericDecimalValidator(control: AbstractControl) {
    ...
    return numericDecimal.test(valueWithoutSpaces)
      ? null
      : { numericDecimal: true };
  }
  private nameExistsValidator(): any {
    return (control: AbstractControl) => {
      if (
        ...
        control.value === this.originalCenterName
      ) {
  ...
  private contactNumbersMatchValidator = (
    ...
    if (
      ...
      contact1Control.setErrors({
        ...contact1Control.errors,
        sameContactNumbers: true,
      });
      contact2Control.setErrors({
        ...contact2Control.errors,
        sameContactNumbers: true,
      });
  ...
  private phoneFormatValidator = (
    ...
    if (
      contact1Control &&
      contact1Control.value &&
      contact1CodeControl &&
      contact1CodeControl.value === '+94'
    ) {
    ...
    if (
      contact2Control &&
      contact2Control.value &&
      contact2CodeControl &&
      contact2CodeControl.value === '+94'
    ) {
      const phoneRegex = /^7\d{8}$/;
  ...
  updateDistributionCentre() {
    ...
    if (
      this.distributionForm.get('latitude')?.value &&
      this.getFieldError('latitude') === 'Latitude must be between -90 and 90'
    ) {
  ...
  private formatErrorMessagesForAlert(
    ...
    html +=
      '<p class="mb-3 font-semibold">Please correct the following errors:</p>';
  ...
  onInputChange(event: any, fieldType: string) {
    ...
    switch (fieldType) {
      ...
      case 'coordinates':
        ...
        this.distributionForm
          .get(target.getAttribute('formControlName')!)
          ?.setValue(valueWithoutSpaces);

        if (
          valueWithoutSpaces === '' &&
          coordOriginalValue.length > 0 &&
          !/[0-9.-]/.test(coordOriginalValue)
        ) {
          shouldUpdate = false;
        }
  ...
  onPhoneInputChange(event: any, fieldName: string) {
    ...
    if (value === '' || value.charAt(0) === '7') {
      ...
      this.distributionForm
        .get(fieldName)
        ?.setValue(value.startsWith('7') ? value : '');
    }
