...
export class PlantcareUsersRowComponent implements OnChanges {
  ...
  fetchPlantCareUserData(data: any) {
    const qrFarmers = data?.QRfarmers || {
      notQrCode: { count: 0 },
      QrCode: { count: 0 },
    };
    ...
    this.allPlantCareUsers = this.totCount(
    ...
    this.QRpresentageForOutput =
      total > 0
        ? ((this.plantCareUsersWithQr / total) * 100).toFixed(1)
        : '0.0';
    this.nonQRpresentageForOutput =
      total > 0
    ...
    this.plantCareDataEmitted.emit({
      ...
      QRpresentageForOutput: this.QRpresentageForOutput,
      nonQRpresentageForOutput: this.nonQRpresentageForOutput,
    });
  }
