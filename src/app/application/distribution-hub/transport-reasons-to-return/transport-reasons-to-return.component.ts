...
export class TransportReasonsToReturnComponent implements OnInit {
  ...
  loadReasons(): void {
    this.isLoading = true;
    this.distributionService.getAllReturnReasons().subscribe({
      next: (response) => {
  ...
  onSubmit(): void {
    ...
    // Create new reason
    this.isLoading = true;
    this.distributionService.createReturnReason(reasonData).subscribe({
      next: (response) => {
        this.isLoading = false;
        if (response.status) {
          Swal.fire({
  ...
  onDelete(id: number): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.isLoading = true;
        this.distributionService.deleteReturnReason(id).subscribe({
          next: (response) => {
            this.isLoading = false;
            if (response.status) {
              Swal.fire({
  ...
  private saveIndexesToBackend(): void {
    ...
    this.distributionService.updateReturnReasonIndexes(indexData).subscribe({
      next: (response) => {
        if (response.status) {
