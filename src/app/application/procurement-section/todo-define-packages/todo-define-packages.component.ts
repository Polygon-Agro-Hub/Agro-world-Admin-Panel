...
export class TodoDefinePackagesComponent implements OnInit {
  ...
  fetchOrders(): void {
    ...
    // Pass all relevant filters to the service
    this.orderService
      ...
      .subscribe({
        next: (response) => {
          ...
          // Make sure the response structure matches expectations
          if (response && response.data) {
            this.orders = response.data;
            this.totalItems = response.total || response.totalCount || 0; // Handle different property names
          } else {
